...
export default function InvoicesPage() {
  ...
  // Only includes clients with webflow_cost > 0 (excludes defunct clients)
  ...
  const calculateMRR = () => {
    ...
    // Only sum fees for active hosting clients WITH Webflow costs (excludes defunct)
    const activeHostingClients = new Set(
      hostingProjects
        .filter(p => p.status === 'active' && p.webflow_cost > 0)
        .map(p => p.client_id)
    )
