...
export default function ProjectDetailsPage() {
  ...
  const handleCopyLink = async () => {
    const shareUrl = `${window.location.origin}/projects/${id}?view=1`
    try {
  ...
  const handleChunkDelete = (chunkId) => {
    setConfirmModal({
      title: 'Delete Chunk',
      message: 'Delete this chunk? This cannot be undone.',
      confirmText: 'Delete',
      danger: true,
      onConfirm: async () => {
        try {
          ...
          setErrorModal({ title: 'Error', message: 'Failed to delete chunk. Please try again.' })
        }
      }
    })
  }
  ...
  const handleArchive = () => {
    ...
    setConfirmModal({
      ...
      message: isArchived
        ? `Unarchive "${project.name}"? This will restore the project to your active projects list.`
      ...
      onConfirm: async () => {
        try {
          ...
          setErrorModal({ title: 'Error', message: `Failed to ${action} project. Please try again.` })
        }
      }
    })
  }
  ...
  const handleDelete = () => {
    if (project.status !== 'archived') {
      setErrorModal({
        title: 'Cannot Delete',
        message: 'Projects must be archived before they can be deleted.'
      })
      return
    }
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          ...
          setErrorModal({ title: 'Error', message: 'Failed to delete project. Please try again.' })
        }
      }
    })
  }
  ...
  return (
    <div className={`p-6 max-w-6xl mx-auto ${isViewMode ? 'pb-16' : ''}`}>
      ...
      {project.status === 'archived' && !isViewMode && (
        <div className="mb-6 -mt-2 p-4 bg-amber-50 border border-amber-200 rounded-lg flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Archive className="w-5 h-5 text-amber-600" />
            <div>
              <p className="font-medium text-amber-800">This project is archived</p>
              <p className="text-sm text-amber-600">It won't appear in your active projects list.</p>
            </div>
          </div>
          <button
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            {project.proposal_id && !isViewMode && (
              <Link
                to={`/proposals/${project.proposal_id}/edit`}
      ...
      {errorModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-md shadow-xl">
            ...
            <div className="p-4">
              <p className="text-slate-600 whitespace-pre-wrap">{errorModal.message}</p>
            </div>
            <div className="flex justify-end p-4 border-t border-slate-100 bg-slate-50 rounded-b-xl">
              <button
