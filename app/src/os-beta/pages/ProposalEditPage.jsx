...
import { Printer, Check, Palette, Layout, Smartphone, BarChart3, Shield, Zap, Megaphone, RefreshCw, PenTool, Plus, Trash2, Calendar, Percent, LinkIcon } from 'lucide-react'

const API_BASE = import.meta.env.DEV ? 'http://localhost:3002/api/os-beta' : '/api/os-beta'
const TEMPLATES_API = import.meta.env.DEV ? 'http://localhost:3002/api' : '/api'
...
const MarkdownContent = ({ content }) => {
  ...
  lines.forEach((line, i) => {
    ...
    if (trimmed.startsWith('### ')) { flushList(); elements.push(<h5 key={i} className="font-semibold text-slate-800 mt-6 mb-2">{trimmed.slice(4)}</h5>); return }
    if (trimmed.startsWith('## ')) { flushList(); elements.push(<h4 key={i} className="font-bold text-slate-900 mt-6 mb-3 text-base">{trimmed.slice(3)}</h4>); return }
    if (trimmed.startsWith('# ')) { flushList(); elements.push(<h3 key={i} className="font-bold text-slate-900 mt-6 mb-3 text-lg">{trimmed.slice(2)}</h3>); return }
    if (trimmed.startsWith('- ') || trimmed.startsWith('* ') || trimmed.startsWith('• ')) { currentList.push(trimmed.slice(2)); return }
    flushList()
...
export default function ProposalEditPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`${TEMPLATES_API}/templates`).then(r => r.json())
    ])
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-6 print:bg-white print:py-0">
      ...
      <div className="no-print fixed top-4 right-4 flex items-center gap-2 z-50">
        {saving && <span className="text-xs text-slate-400 bg-white px-2 py-1 rounded">Saving...</span>}
        <button
